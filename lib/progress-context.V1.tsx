...
interface ProgressContextType {
    ...
    importData: (data: UserProgress) => void; // [NEW]
}
...
export function ProgressProvider({ children }: { children: React.ReactNode }) {
    ...
    useEffect(() => {
        const init = async () => {
            try {
                ...
                if (session?.user) {
                    console.log('User logged in, fetching from DB:', session.user.id);
                    ...
                    if (data && data.data && data.data.xp > 0) {
                        console.log('Loaded progress from Cloud, xp:', data.data.xp);
                        initialProgress = data.data;
                    } else {
                        // Cloud Empty or Missing: Check for local data
                        console.log('Cloud data empty or missing. Checking local...');
                        const local = loadProgress();

                        if (local.xp > 0) {
                            console.log('Found local progress, syncing to Cloud (Healing)');
                            initialProgress = local;
                            // Immediate sync to fix cloud
                            await supabase
                                .from('user_progress')
                                .upsert({ user_id: session.user.id, data: local });
                        } else if (data && data.data) {
                            // Both empty, stick with cloud default
                            initialProgress = data.data;
                        } else {
                            // New user, no data anywhere
                            await supabase
                                .from('user_progress')
                                .insert({ user_id: session.user.id, data: initialProgress });
                        }
                    }
                } else {
                ...
                console.error('ProgressContext init error:', err);
            } finally {
                setIsLoading(false);
            }
        };
        ...
        const { data: { subscription } } = supabase.auth.onAuthStateChange(async (_event, session) => {
            ...
            // Only react to actual user-initiated auth changes
            if (_event === 'INITIAL_SESSION') {
                console.log('Skipping INITIAL_SESSION (handled by init)');
                return;
            }
            ...
            try {
                if (newUser) {
                    // Fetch this user's data from cloud
                    const { data, error } = await supabase
                        .from('user_progress')
                        .select('data')
    ...
    // 2. Save Progress (Cloud OR Local, not both)
    useEffect(() => {
        if (!isLoading) {
            if (user) {
                // LOGGED IN: Sync to cloud ONLY (no localStorage to avoid data bleed)
                const sync = async () => {
    ...
    const importData = useCallback((data: UserProgress) => {
        setProgress(data);
    }, []);
